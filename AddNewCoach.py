...
class AddnewCoach(BaseFunc):
    ...
    PREFFERED_DAYS_ELE='//div//label[text()="Preffered Days *"]'
    # SELECT_ALL_VAL='//ul[@aria-labelledby="mutiple-select-label"]/li[2]'
    SELECT_ALL_VAL ='//span[@class="MuiTypography-root MuiListItemText-primary MuiTypography-body1 MuiTypography-displayBlock" and contains(text(),"day")]'
    PREFFERED_TIME_ELE='//div[@aria-labelledby="mutiple-select-label"]'
    P_T_VAL='//ul[@aria-labelledby="mutiple-select-label"]/li[3]'
    ...
    def get_p_d(self):
        # return self.driver.find_element(By.XPATH, self.SELECT_ALL_VAL)
        wait=WebDriverWait(self.driver,5)
        return WebDriverWait(self.driver, 10).until(EC.presence_of_all_elements_located((By.XPATH, self.SELECT_ALL_VAL)))
    def p_d(self):
        self.get_p_d().click()
        time.sleep(10)
